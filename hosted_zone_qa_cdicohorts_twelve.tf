...
provider "aws" {
  alias  = "subdomain_qa_cohorts_twelve"
  region = "us-east-1"
  assume_role {
...
module "subdomain_qa_cohorts_twelve" {
  source  = "terraform-aws-modules/route53/aws//modules/zones"
  version = "2.0.0"
  create  = true

  providers = {
    aws = aws.subdomain_qa_cohorts_twelve
  }
...
module "subdomain_zone_delegation_qa_cohorts_twelve" {
  ...
  records = [
    {
      ...
      records         = lookup(module.subdomain_qa_cohorts_twelve.route53_zone_name_servers,"qa.${local.domain_cdicohorts_twelve}")
    }
  ]

  depends_on = [module.subdomain_qa_cohorts_twelve]
}
