...
provider "aws" {
  alias  = "subdomain_qa_cdicohorts_twelve"
...
module "subdomain_qa_cdicohorts_twelve" {
  source  = "terraform-aws-modules/route53/aws//modules/zones"
  version = "2.0.0"
  create  = true

  providers = {
    aws = aws.subdomain_qa_cdicohorts_twelve
  }

  zones = {
    "qa.${local.domain_cdicohorts_twelve}" = {
      tags = {
        cluster = "nonprod"
      }
    }
...
module "subdomain_zone_delegation_qa_cdicohorts_twelve" {
  ...
  records = [
    {
      ...
      records         = lookup(module.subdomain_qa_cdicohorts_twelve.route53_zone_name_servers,"qa.${local.domain_cdicohorts_twelve}")
    }
  ]

  depends_on = [module.subdomain_qa_cdicohorts_twelve]
}
